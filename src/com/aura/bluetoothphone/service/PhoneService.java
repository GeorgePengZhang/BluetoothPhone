...
import com.aura.bluetoothphone.utils.calling.Overlay;
import com.aura.bluetoothphone.utils.calling.Utils;
...
public class PhoneService extends Service {
	...
	private BroadcastReceiver mBroadcastReceiver = new BroadcastReceiver() { // 监听
																				// 来电、去电广播
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (intent.getAction().equals(Intent.ACTION_NEW_OUTGOING_CALL)) {
				...
				LogUtil.out("call OUT" + phoneNumber);
				...
				case TelephonyManager.CALL_STATE_RINGING: // 响铃来电 来电号码
					mIncomingFlag = true;
					mIncomingNumber = intent.getStringExtra("incoming_number");
					LogUtil.out("RINGING :" + mIncomingNumber);
					try {
						new Handler().postDelayed(new Runnable() {
							@Override
							public void run() {
//								 OverlayView.show(getBaseContext(),
//								 mIncomingNumber, 100);
								 OverlayView view = new OverlayView() ;
								 OverlayView.mOverlayView mView = view.new
								 mOverlayView() ;
								 mView.show(getBaseContext(), mIncomingNumber,
								 100);
...
//								getBaseContext().startActivity(intent);
				...
				case TelephonyManager.CALL_STATE_OFFHOOK: // 接听
					if (mIncomingFlag) {
						LogUtil.out("incoming ACCEPT :" + mIncomingNumber);
					}
					break;
				case TelephonyManager.CALL_STATE_IDLE: // 挂断
					 closeWindow(getBaseContext()) ;
					if (mIncomingFlag) {
						LogUtil.out("incoming IDLE");
					}
					break;
				}
			}
	...
	private void closeWindow(Context ctx) {
		OverlayView view = new OverlayView();
		OverlayView.mOverlayView mView = view.new mOverlayView();
		mView.hide(ctx);
	}
	...
	class OverlayView {
		class mOverlayView extends Overlay {
			...
			private final int DTMF_DURATION_MS = 120; // 声音的播放时间
			public final int NUM_ONE = 1;
			public final int TELE_START_TIME = 2; // 开始录音了
			public final int TELE_END_TIME = 3; // 录音结束
			public final int TELE_FAILURE_TIME = 4; // 录音失败
			private String phoneNumber = ""; // 电话号码
			private MediaRecorder recorder = null; // 录音控件
			private File root_file; // 录音保存文件夹
			private File file;
			private String root_directory; // 文件保存路径
			/** 上次按下的时间 */
			private long lastExitTime = 0;

			private String[] mNotifacations = { "你好,我现在有事,一会再联系你！",
					"正在开会，稍后回复你！", "现在不方便接电话！", "在吃饭，一会打给你！",
					"在外面不方便接电话，稍后回复你！" };
			 ...
			 * @param number
			 */
			public void show(Context context, String number, int percentScreen) {
				synchronized (monitor) {
					mContext = context;
					init(context, number, R.layout.activity_calling,
							percentScreen);
				}
			}
			 ...
			 * @return 布局
			 */
			private ViewGroup init(Context context, String number, int layout,
					int percentScreen) {
			 ...
			 * 初始化界面
			 */
			private void initView(View v, String phoneNum, int percentScreen) {
				...
				view_call_phone = (LinearLayout) v
						.findViewById(R.id.view_call_phone);
				view_keyboard = (LinearLayout) v
						.findViewById(R.id.view_keyboard);
				image_head = (CircleImageView) v
						.findViewById(R.id.image_head_imageView);
				txt_phone_number = (TextView) v
				...
				imageBut_answer = (ImageButton) v
						.findViewById(R.id.imageBut_answer);
				imageBut_reject = (ImageButton) v
						.findViewById(R.id.imageBut_reject);
				imageBut_contents = (ImageButton) v
						.findViewById(R.id.imageBut_contents);
				imageBut_keys = (ImageButton) v
						.findViewById(R.id.imageBut_keys);
				imageBut_recording = (ImageButton) v
						.findViewById(R.id.imageBut_recording);
				imageBut_volume = (ImageButton) v
						.findViewById(R.id.imageBut_volume);
				imageBut_hangup = (ImageButton) v
						.findViewById(R.id.imageBut_hangup);
			}
			 ...
			 * @return
			 */
			private int getHeight(Context context, int percentScreen) {
				return getLarger(context) * percentScreen / 100;
			}

			@SuppressWarnings("deprecation")
			private int getLarger(Context context) {
				WindowManager wm = (WindowManager) context
						.getSystemService(Context.WINDOW_SERVICE);
			...
			private int getLarger(Display display) {
			...
			Handler myHandler = new Handler() {
				public void handleMessage(Message msg) {
					...
					case CallingActivity.NUM_ONE:
						txt_number_and_countdown
								.setText(getStringTime(mCountdown++));
						break;
			...
			private String getStringTime(int cnt) {
				...
				int min = cnt % 3600 / 60;
				int second = cnt % 60;
				return String.format(Locale.CHINA, "%02d:%02d:%02d", hour, min,
						second);
			}
			...
			private void onClick() {
				imageBut_volume.setOnClickListener(new OnClickListener() { // 扩音
							...
							public void onClick(View v) {
								if (mSpeekModle) {
									StringUtil.setSpeekModle(mContext,
											mSpeekModle);
									imageBut_volume
				...
				imageBut_answer.setOnClickListener(new OnClickListener() {// 接听
																			// 来电
				...
				imageBut_reject.setOnClickListener(new OnClickListener() { // 拒接
																			// 来电
			...
			private void playTone(int tone) {
				...
				synchronized (mToneGeneratorLock) {
					if (mToneGenerator == null) {
						LogUtil.out("TAG",
								"playTone: mToneGenerator == null, tone: "
										+ tone);
						return;
					}
			...
			private void setEditText(String num) {
				...
				StringBuilder builder = new StringBuilder(txt_phone_number
			...
			public void recorDing(String phoneNumber, boolean mRecording) {
				...
				if (mRecording) {
					try {
						...
						String record_call = root_directory + "/" + phoneNumber
								+ "_" + System.currentTimeMillis() + ".amr";
			...
			private void setNotifacation() {
				...
				builder.setSingleChoiceItems(mNotifacations,
						R.layout.msm_dialog_item,
				...
				builder.setPositiveButton("确定",
						new DialogInterface.OnClickListener() {
							...
							public void onClick(DialogInterface dialog,
								...
								ToastUtil.showToast(mContext,
										mNotifacations[mRingPosition - 1]);
							}
						});
